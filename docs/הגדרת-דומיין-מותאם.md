...
- **redirect_uri_mismatch (גוגל)** – הוסף ב-Google Cloud Console → Credentials → OAuth Client גם את ה-URI של Supabase ברירת המחדל: `https://gctdhjgxrshrltbntjqj.supabase.co/auth/v1/callback` (בנוסף לדומיין המותאם). כך גם לפני שהדומיין מאומת וגם אחרי – האימות יעבוד.
- **"An unexpected error occurred" בהתחברות** – אם הדומיין המותאם עדיין לא מאומת, החזר זמנית את `VITE_SUPABASE_URL` ל-`https://gctdhjgxrshrltbntjqj.supabase.co` ב-Vercel וב-`.env`.

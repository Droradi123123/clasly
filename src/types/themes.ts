...
// Forbidden light/white backgrounds for quiz options - never use white
const FORBIDDEN_LIGHT_BG = /\bbg-(white|neutral-100|neutral-200|gray-100|gray-200|slate-100|slate-200|zinc-100|zinc-200|stone-100|stone-200)\b/;
const FALLBACK_OPTION_COLOR = 'bg-neutral-600 text-white hover:bg-neutral-500';

/** Returns a safe option color - never white or very light background */
export function getSafeOptionColor(theme: Theme, index: number): string {
  const color = theme.optionColors[index % theme.optionColors.length];
  if (FORBIDDEN_LIGHT_BG.test(color)) return FALLBACK_OPTION_COLOR;
  return color;
}

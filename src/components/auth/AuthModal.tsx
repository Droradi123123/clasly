...
export const AuthModal = ({ isOpen, onClose, onSuccess, promptText, redirectTo = 'dashboard' }: AuthModalProps) => {
  ...
  // Listen for auth state changes to handle post-login navigation
  useEffect(() => {
    if (!isOpen) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      // Handle successful sign in (both email and OAuth)
      if (event === 'SIGNED_IN' && session && !hasHandledAuth.current) {
        ...
        <div className="relative bg-gradient-to-br from-primary/10 via-primary/5 to-transparent px-6 pt-8 pb-6">
          ...
          >
            ...
            <p className="text-muted-foreground text-sm">
              {promptText 
                ? "Sign in to start creating your AI-powered presentation" 
                : "Sign in to create interactive presentations in seconds"
              }
            </p>
          </motion.div>
        </div>
